...
const login = (req, res, next) => {
  ...
  UserModel.findOne({ email })
    ...
    .then(([user, isValidHash]) => {
      ...
      // В.1.: Отправляем токен + ответ пользователю.
      // Токен пользователь должен будет передавать в заголовках запроса
      res.status(200).send({
        name, about, avatar, email, token,
      }); // Отправляем всю информацию о новом пользователе
      // res.status(200).send({ email, token }); // Или просто email и токен
      ...
      //   // sameSite: true, // посылать куки, только если запрос с того же домена, где сервер
