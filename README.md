...
### Бэкенд - `backend/`
  ...
  > * Настроить проверку приходящих от пользователя данных - валидировать данные на входе и заменять управляющие символы мнемониками. Можно реализовать с помощью модуля `escape-html`.
...
> При установке и запуске текущего проекта локально, если адрес и название базы MongoDB у вас не отличаются, создание файла `.env` не обязательно.
> Также на локальном сервере вместо использования переменных окружения можно задать значения по умолчанию для `MONGO_DB` и `PORT` в файле `backend/app.js`.
